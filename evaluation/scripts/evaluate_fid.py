...
class ImageDataset(Dataset):
    def __init__(self, paths, image_size): self.paths = paths; self.image_size = image_size
...
def parse_args():
    ...
    p.add_argument("--image-size", type=int, default=299, help="Resize all images to this square size before FID batching.")
    p.add_argument("--per-class", action="store_true")
    return p.parse_args()

def update(metric, paths, real, args):
    loader = DataLoader(ImageDataset(paths, args.image_size), batch_size=args.batch_size, num_workers=args.workers)
...
def main():
    ...
    result = {"metric":"FID", "feature_dimension":args.feature, "image_size":args.image_size, "real_image_count":len(real_all), "generated_image_count":len(generated_all), "overall_fid":calculate(real_all, generated_all, args), "notes":"Lower is better. Images are resized before batching; use equal sample counts and identical settings when comparing runs."}
    if args.per_class:
