...
def parse_args() -> argparse.Namespace:
    """Parse command line arguments for generating images from a model run."""
    parser = argparse.ArgumentParser(
        description="Generate images for a named evaluation model run."
    )
    parser.add_argument(
        "run_name",
        help="Run name from evaluation/configs/model_runs.json."
    )
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("evaluation/configs/model_runs.json"),
        help="Path to model runs configuration file"
    )
    parser.add_argument(
        "--num-images",
        type=int,
        help="Override protocol image count per class."
    )
    parser.add_argument(
        "--height",
        type=int,
    ...
    parser.add_argument(
        "--num-inference-steps",
        type=int,
        help="Override protocol inference steps."
    )
    parser.add_argument(
        "--guidance-scale",
        type=float,
        help="Override protocol guidance scale."
    )
    parser.add_argument(
        "--seed",
        type=int,
    ...
    parser.add_argument(
        "--disable-xet",
        action="store_true",
        help="Disable XET for Hugging Face downloads"
    )
    parser.add_argument(
        "--hf-transfer-workers",
        type=int,
        default=1,
        help="Number of workers for Hugging Face transfer"
    )
    return parser.parse_args()


def load_runs(path: Path) -> dict[str, Any]:
    """Load and validate model runs configuration from a JSON file.

    Args:
        path: Path to the model runs JSON file.

    Returns:
        Dictionary containing model run configurations.

    Raises:
        SystemExit: If file not found, invalid JSON, or not a dictionary.
    """
    try:
...
def append_optional(command: list[str], flag: str, value: object | None) -> None:
    """Append a command line flag with its value if the value is not None.

    Args:
        command: List of command arguments to extend.
        flag: Command line flag (e.g., "--num-images").
        value: Value to append after the flag.
    """
...
def main() -> int:
    """Build and execute the generation command for a named model run.
    ...
    if run["pipeline_type"] in {"sdxl_lora", "sd3_lora"}:
        ...
        raise SystemExit(
            f"Unsupported pipeline_type for {args.run_name}: {run['pipeline_type']}"
        )
